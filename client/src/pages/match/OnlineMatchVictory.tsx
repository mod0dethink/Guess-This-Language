...
/**
 * OnlineMatchVictoryコンポーネント
 * 勝利時の結果画面を表示
 * location.stateまたはLocalStorageから対戦結果データを取得
 * 再戦またはホームへの遷移ボタンを提供
 */
const OnlineMatchVictory = () => {
  ...
  const yourScore = state.yourScore ?? lastMatch.yourScore ?? 2450;
  const opponentScore = state.opponentScore ?? lastMatch.opponentScore ?? 1890;
  ...
  return (
    <MatchResultScreen
      ...
      subline={
        <>
          Won <span className="text-white">{yourCorrect} – {oppCorrect}</span> against{" "}
...
export default OnlineMatchVictory;
